...
import numpy as np
from core.common import itertools_utils as iu

import core.common.aggregate_iterable
...
class IterToolsTestCase(unittest.TestCase):
    ...
    def test_aggregate_iterable_int_range_without_count(self):
        x = range(10)
        aggregated = core.common.aggregate_iterable.aggregate_iterable(x, detect_final_shape_by_first_elem=True)
        truth_val = np.arange(10)
        self.assertTrue((truth_val == aggregated).all())

    def test_aggregate_iterable_int_range_with_count(self):
        x = range(10)
        aggregated = core.common.aggregate_iterable.aggregate_iterable(x, n_elements=10, detect_final_shape_by_first_elem=True)
        truth_val = np.arange(10)
        self.assertTrue((truth_val == aggregated).all())
